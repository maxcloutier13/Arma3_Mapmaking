...
_hostile_marker_class = "SignAd_SponsorS_Vrana_F";
...
while { cqb_1_hostile_count <= _target_count } do {
	...
	_unit addEventHandler ["HIT", {
		...
		cqb_1_hostile_count = cqb_1_hostile_count - 1;
		if (cqb_1_hostile_count == 0) then {
			hint "Good job Viper";
		} else {
			hint format ["%1 cibles restantes", cqb_1_hostile_count];
		};
	}];
	// count
	cqb_1_hostile_count = cqb_1_hostile_count + 1;
