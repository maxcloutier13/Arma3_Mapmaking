...
_hostile_marker_class = "SignAd_SponsorS_Redstone_F";
...
{
	deleteVehicle _x
} forEach cqb_2_target_list;

// Clears variables 
cqb_2_target_list = [];
cqb_2_hostile_count = 0;
...
while { cqb_2_hostile_count <= _target_count } do {
	...
	_unit addEventHandler ["HIT", {
		...
		cqb_2_hostile_count = cqb_2_hostile_count - 1;
		if (cqb_2_hostile_count == 0) then {
			hint "Good job Viper";
		} else {
			hint format ["%1 cibles restantes", cqb_2_hostile_count];
		};
	}];
	// count
	cqb_2_hostile_count = cqb_2_hostile_count + 1;
	cqb_2_target_list pushBack _unit;
};