...
_hostile_marker_class = "SignAd_SponsorS_ARMEX_F";
...
{
	deleteVehicle _x
} forEach cqb_3_target_list;

// Clears variables 
cqb_3_target_list = [];
cqb_3_hostile_count = 0;
...
while { cqb_3_hostile_count <= _target_count } do {
	...
	_unit addEventHandler ["HIT", {
		...
		cqb_3_hostile_count = cqb_3_hostile_count - 1;
		if (cqb_3_hostile_count == 0) then {
			hint "Good job Viper";
		} else {
			hint format ["%1 cibles restantes", cqb_3_hostile_count];
		};
	}];
	// count
	cqb_3_hostile_count = cqb_3_hostile_count + 1;
	cqb_3_target_list pushBack _unit;
};