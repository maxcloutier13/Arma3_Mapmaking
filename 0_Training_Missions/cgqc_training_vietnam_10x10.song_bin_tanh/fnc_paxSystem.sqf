...
switch (_type) do {
	case 1:	{ // Turn it on
		...
		distancesArray = [];
		{
			...
			_distance = playerPos distance locationPos;
		...
		_patrols = [1,2,3];
		{
			...
			_randomPos = [cgqc_pax_hqOpfor_pos, 10, 50, 5, 0, 0, 0, [], []] call BIS_fnc_findSafePos;
			_patrolName  = [_randomPos, east, cgqc_pax_opfor_patrol] call BIS_fnc_spawnGroup;
